...
module.exports = {
  ...
  logCommands: async function (message) {
    ...
    await pool
      .query(
        'INSERT INTO commands (command, user_id, username, server_id, date_added) VALUES (?, ?, ?, ?, ?)',
        [
          message.content,
          message.author.id,
      ...
      .then(function (res) {
        if (res.insertId > 0) {
          ...
          pool.query('UPDATE commands SET hash = ? WHERE id = ?', [
            hashid.encode(res.insertId),
            res.insertId,
          ]);
  ...
  owner: function (member) {
    if (Object.keys(config.adminIDs).includes(member.id)) return true;
    else return false;
  },
};
