...
pool.query("SELECT server_agents.*, platforms.platform  FROM `server_agents` LEFT JOIN platforms ON server_agents.server_id = platforms.server_id WHERE agent_id NOT IN (SELECT uplay_id FROM users)").then(async function(response){
